...
export const uploadFile = async ({ filePath, fileName, storage, resize, isImage, withThumbImage, options: { file, onError, onProgress, onSuccess }, }) => await new Promise((resolve, reject) => {
    ...
    storage
        ...
        .put(file)
        .on("state_changed", ({ bytesTransferred, totalBytes }) => onProgress((bytesTransferred / totalBytes) * 95), (error) => {
        onError(error);
        reject(error);
...
export const deleteFileAndFileThumbFromStorage = async (storage, filePath, fileName) => {
    const extension = fileName.split(".").pop();
    const pathImage = `${filePath}/${fileName}`;
    const pathThumbImages = imageResizes.map((resizeImage) => `${filePath}/thumbs/${fileName.replace(`.${extension}`, "")}_${resizeImage}.${extension}`);
    const uris = [pathImage, ...pathThumbImages].map((url) => `gs://${storage.ref().bucket}/${url}`);
    try {
