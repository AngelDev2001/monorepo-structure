...
export const addUser = async (user: User): Promise<FirebaseFirestore.WriteResult> =>
  setDocument<User>(usersRef.doc(user.id), user);

export const fetchUserByDocument = async (userDocument: string): Promise<User[] | undefined> =>
  fetchCollection<User>(
    usersRef.where('isDeleted', '==', false).where('document.number', '==', userDocument).limit(1)
  );
